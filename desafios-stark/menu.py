# MENU DE OPCIONES
def mostrar_menu():
    """
    Muestra el menú de opciones para el programa.
    """
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║                            MENU                              ║")
    print("║                                                              ║")
    print("║  1. Mostrar nombres de los superheroes.                      ║")
    print("║  2. Mostrar nombre y altura de los superheroes.              ║")
    print("║  3. El superheroe mas alto.                                  ║")
    print("║  4. El superheroe mas bajo.                                  ║")
    print("║  5. Altura promedio de los superheroes.                      ║")
    print("║  6. Nombre del superheroe mas alto y mas bajo.               ║")
    print("║  7. Superheroe mas y menos pesado.                           ║")
    print("║  8. Mostrar superheroes masculinos.                          ║")
    print("║  9. Mostrar superheroes femeninos.                           ║")
    print("║ 10. El superheroe masculino mas alto.                        ║")
    print("║ 11. El superheroe femenino mas alto.                         ║")
    print("║ 12. El superheroe masculino mas bajo.                        ║")
    print("║ 13. El superheroe femenino mas bajo.                         ║")
    print("║ 14. Altura promedio de los superheroes masculinos.           ║")
    print("║ 15. Altura promedio de los superheroes femeninos.            ║")
    print("║ 16. Superheroes mas bajos y altos por genero.                ║")
    print("║ 17. Mostrar cuantos hay segun color de ojos.                 ║")
    print("║ 18. Mostrar cuantos hay segun color de pelo.                 ║")
    print("║ 19. Mostrar cuantos hay segun inteligencia.                  ║")
    print("║ 20. Lista segun color de ojos.                               ║")
    print("║ 21. Lista segun color de pelo.                               ║")
    print("║ 22. Lista segun inteligencia.                                ║")
    print("║  0. Salir                                                    ║")
    print("╚══════════════════════════════════════════════════════════════╝")

