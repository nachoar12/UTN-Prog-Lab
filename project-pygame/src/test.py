...
def load_coin_list(coins, cantidad):
    for i in range(cantidad):
        ...
        coins.append(crear_bloque(None, randint(0, WIDTH - size_coin), randint(0, HEIGHT - size_coin), size_coin, size_coin, yellow, radio=size_coin // 2))
...
while is_running:
    ...
    for e in pygame.event.get():
        ...
        if e.type == MOUSEBUTTONDOWN:
            if e.button == 1:
                new_coin = (crear_bloque( e.pos[0] , e.pos[1], size_coin, cyan, radio= size_coin // 2))
    ...
    VENTANA.fill(background, (0,0))
    for coin in coins:
        pygame.draw.rect(VENTANA, coin["color"], coin["rect"], coin["borde", coin["radio"]])
